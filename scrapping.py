...
scrapfly = ScrapflyClient(key="scp-live-85ebebbf20be4fafac3df58875af8483", max_concurrency=2)
...
async def scrape_search(url: str, max_pages: int = 2) -> List[str]:
    ...
    for proprety urls
    """
async def scrape_search(url: str, paginate: bool = True) -> List[str]:
    """
    Scrape search urls like:
    https://www.idealista.com/en/venta-viviendas/marbella-malaga/con-chalets/
    for property urls
    """
    max_pages = 60  # Define el límite máximo de páginas permitidas por búsqueda
